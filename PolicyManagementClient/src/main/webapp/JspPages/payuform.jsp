...
<input type="hidden" name="key" value="<%= merchant_key %>" />
      ...
      <table>
        ...
        <tr>
          <td>Amount: </td>
          <td><input name="amount" value="<%= (empty(params.get("amount"))) ? session.getAttribute("premium") : params.get("amount") %>" /></td>
          <td>First Name: </td>
          <td><input name="firstname" id="firstname" value="<%= session.getAttribute("name") %>" /></td>
        </tr>
        <tr>
          <td>Email: </td>
          <td><input name="email" id="email" value="<%= (empty(params.get("email"))) ? session.getAttribute("email") : params.get("email") %>" /></td>
          <td>Phone: </td>
          <td><input name="phone" value="<%=session.getAttribute("mobile") %>" /></td>
        </tr>
        <tr>
          <td>Product Info: </td>
        ...
        <tr>
          <td>Success URI: </td>
          <td colspan="3"><input type="hidden" name="surl" value="http://localhost:8098/SuccessfullyPay.jsp" size="64" /></td>
        </tr>
        <tr>
          <td>Failure URI: </td>
          <td colspan="3"><input type="hidden" name="furl" value="https://fail-url.in" size="64" /></td>
        </tr>
        <tr>
        ...
        <tr>
          <td>Address1: </td>
          <td><input name="address1" value="<%= (empty(params.get("address1"))) ? session.getAttribute("city") : params.get("address1") %>" /></td>
          <td>Address2: </td>
          <td><input name="address2" value="<%= (empty(params.get("address2"))) ? "" : params.get("address2") %>" /></td>
        </tr>
        <tr>
          <td>City: </td>
          <td><input name="city" value="<%=session.getAttribute("city") %>" /></td>
          <td>State: </td>
